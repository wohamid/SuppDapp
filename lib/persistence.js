...
export const createProject = async ({ contract, name, owner, origin }) => {
    const project = {
        name,
        owner,
        origin,
...
export const verifyProject = async (contract) => {
    ...
    const updatedValue = { ...project, verified: false };
...
export const getTicketsForOwner = async (projectOwner) => {
    ...
    for (const key in keys) {
        const owner = await getById(keys[0]);
...
export const resetOwnerKeys = async (projectOwner) => {
    ...
    for (const key in keys) {
        const ticket = await deleteTicketByKey(keys[0]);
    }
}
